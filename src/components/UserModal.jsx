...
import { X, MapPin, Github, Star, Mail } from "lucide-react";

export default function UserModal({ user, onClose, isDark }) {
    if (!user) return null;
    const bg = isDark ? "#1a1a28" : "#fff";
    ...
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.65)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(4px)" }} onClick={onClose}>
            <div style={{ width: 440, maxWidth: "90%", background: bg, borderRadius: 24, padding: "28px", color: txt, border: `1px solid ${border}`, boxShadow: "0 24px 64px rgba(0,0,0,0.5)", position: "relative" }} onClick={(e) => e.stopPropagation()}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, fontSize: "0.82rem", color: muted, marginBottom: 24 }}>
                    ...
                    {user.github && <div style={{ display: "flex", alignItems: "center", gap: 8 }}><Github size={15} color="#F0F0FF" />{user.github}</div>}
